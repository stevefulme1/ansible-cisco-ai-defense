...
"""Ansible module for managing Cisco AI Defense model scan."""
...
---
module: model_scan
short_description: Manage model scanning configurations in Cisco AI Defense
description:
    - Manage model scanning configurations in Cisco AI Defense via the Cisco AI Defense REST API.
version_added: "1.0.0"
author:
...
options:
    ...
    model_reference:
        description:
            - The model reference parameter.
        type: str
        required: true
    scan_type:
        description:
            - The scan type parameter.
        type: str
        required: true
    scan_id:
        description:
            - The scan id parameter.
        type: str
        required: false
    state:
        description:
...
EXAMPLES = r"""
- name: Create model scan
  stevefulme1.cisco_ai_defense.model_scan:
    name: "example-value"
    model_reference: "example-value"
    scan_type: "example-value"
    state: present

- name: Delete model scan
  stevefulme1.cisco_ai_defense.model_scan:
    scan_id: "example-value"
    state: absent
"""

RETURN = r"""
model_scan:
    description: Details of the model scan.
    returned: On success.
    type: dict
"""
...
def get_resource(module, api_url, headers):
    ...
    resource_id = module.params.get("scan_id")
    if not resource_id:
        return None
    try:
        url = f"{api_url}/api/v1/model_scan/{resource_id}"
...
def create_resource(module, api_url, headers):
    ...
    if module.params.get("model_reference"):
        payload["model_reference"] = module.params["model_reference"]
    if module.params.get("scan_type"):
        payload["scan_type"] = module.params["scan_type"]
    response = requests.post(
        f"{api_url}/api/v1/model_scan",
...
def update_resource(module, api_url, headers, existing):
    ...
    response = requests.put(
        f"{api_url}/api/v1/model_scan/{resource_id}",
        headers=headers, json=payload,
        verify=module.params["validate_certs"],
        timeout=30,
    )
...
def needs_update(params, existing):
    ...
    updatable = ['name', 'model_reference', 'scan_type']
...
def main():
    module_args = dict(
        ...
        scan_type=dict(type="str", required=True),
        scan_id=dict(type="str"),
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, model_scan=resource)
    ...
    module.exit_json(changed=False, model_scan=existing)
