...
"""Ansible module for managing Cisco AI Defense compliance report."""
...
short_description: Manage compliance reports in Cisco AI Defense
description:
    - Manage compliance reports in Cisco AI Defense via the Cisco AI Defense REST API.
version_added: "1.0.0"
author:
...
def get_resource(module, api_url, headers):
    ...
    resource_id = module.params.get("report_id")
    if not resource_id:
        return None
    try:
        url = f"{api_url}/api/v1/compliance_report/{resource_id}"
...
def create_resource(module, api_url, headers):
    ...
    if module.params.get("report_type"):
        payload["report_type"] = module.params["report_type"]
    if module.params.get("scope"):
        payload["scope"] = module.params["scope"]
    response = requests.post(
        f"{api_url}/api/v1/compliance_report",
...
def update_resource(module, api_url, headers, existing):
    ...
    response = requests.put(
        f"{api_url}/api/v1/compliance_report/{resource_id}",
        headers=headers, json=payload,
        verify=module.params["validate_certs"],
        timeout=30,
    )
...
def needs_update(params, existing):
    ...
    updatable = ['name', 'report_type', 'scope']
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, compliance_report=resource)
    ...
    module.exit_json(changed=False, compliance_report=existing)
